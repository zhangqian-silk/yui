...
`1.0.0-alpha.1` 修复独立转换器对 `home_meta` 等价缩进的误判，保留原 DDL，
按目标定义重建该表并验证所有行不变。运行时存储 1.0 定义及校验和不变，
已转换的 Home 无需再次转换。alpha 系列每次使用新的不可变版本／标签和
独立转换器附件，继续发布到 npm `next`，不覆盖已发布版本、标签或附件。
...
使用精确软件版本 `1.0.0-alpha.1` 主动选择新基线。正式版使用 npm `latest`，
...
发布时将转换包及校验和作为每个 alpha 版本的长期附件，与确切测试过的运行包一同保留；
...
转换器源码、冻结终点样本及专用测试只服务于 alpha 系列的独立交付。
必须先确认最终修正版 alpha 的长期发布附件和校验和可取得且验证通过，才能在 1.0.0
