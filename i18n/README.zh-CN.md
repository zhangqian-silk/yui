...
> **状态：** 1.0.0-alpha.1，采用纯净的存储 1.0 基线。默认更新只执行同一存储
> 主版本内已声明的小版本升级。旧 v37 Home 需要独立的
> [一次性转换](../docs/storage-baseline.zh-CN.md)。
...
状态命令前执行该 launcher 的 `setup`。开发工具不管理全局安装，
不要用全局 `yui` 验证本地修改。真实模型、付费或共享资源测试需要用户明确请求这些资源。
